...
RATING_MAP = {"NR": "NR", "BBB": "BBB", "A": "A", "AA": "AA", "AAA": "AAA"}
...
def load_demo_instruments() -> pd.DataFrame:
    df = pd.read_csv(DATA_DIR / "seed_instruments.csv")
    return df
    ...
    out = pd.DataFrame(
        {
            ...
            "aum_mn": np.nan,
            "quote_source": df.get("source", "Legacy CSV"),
...
def load_legacy_mandates() -> pd.DataFrame:
    ...
    def _map_allowed(text: str) -> str:
        ...
        mapped = []
        for x in raw:
            mapped.append(mapped_classes.get(x, x))
...
def build_seed_frames() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    all_inst["tradable"] = 1
    all_inst["is_demo"] = np.where(all_inst["source"].eq("Legacy CSV"), 0, 1)
