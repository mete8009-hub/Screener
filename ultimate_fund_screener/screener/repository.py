...
def get_refresh_log(limit: int = 50) -> pd.DataFrame:
    return query_df("SELECT run_at, source, status, message, rows_loaded FROM refresh_log ORDER BY id DESC LIMIT ?", (limit,))


def get_app_meta() -> pd.DataFrame:
    return query_df("SELECT key, value FROM app_meta ORDER BY key")
