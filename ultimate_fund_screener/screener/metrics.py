...
def weighted_portfolio_attributes(universe: pd.DataFrame, weights: dict[str, float]) -> dict[str, float]:
    rows = universe[universe["instrument_id"].isin(weights.keys())].copy()
    if rows.empty:
        return {}
    rows["w"] = rows["instrument_id"].map(weights).fillna(0.0).astype(float)
    if rows["w"].sum() <= 0:
        return {}
    rows["w"] = rows["w"] / rows["w"].sum()
    ...
    def wavg(col: str) -> float:
        ...
        if not valid.any():
            return np.nan
        w = rows.loc[valid, "w"]
        return float(np.average(s[valid], weights=w))

    return {
        "Weighted YTM": wavg("ytm_pct"),
        "Weighted Duration": wavg("duration_years"),
        "Weighted ESG": wavg("esg_score"),
        "Weighted Liquidity": wavg("liquidity_score"),
    }
